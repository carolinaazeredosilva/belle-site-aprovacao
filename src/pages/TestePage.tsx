...
const submitErrorByLang: Record<
  TestePageLanguage,
  { generic: string; config: string; network: string; validation: string; emailDelivery: string }
> = {
  pt: {
    generic: "Não foi possível enviar agora. Tente novamente.",
    config: "Configuração de envio indisponível no ambiente online. Contate o suporte.",
    network: "Falha de conexão no envio. Verifique sua internet e tente novamente.",
    validation: "Revise os campos obrigatórios e tente novamente.",
    emailDelivery: "Seus dados foram salvos, mas o e-mail não foi enviado. Contate o suporte para ajustar o disparo.",
  },
  en: {
    ...
    emailDelivery: "Your data was saved, but the email notification failed. Please contact support.",
  },
  es: {
    generic: "No fue posible enviar ahora. Inténtalo de nuevo.",
    config: "La configuración de envío no está disponible en este entorno online. Contacta al soporte.",
    network: "Falló la conexión durante el envío. Revisa tu internet e inténtalo de nuevo.",
    validation: "Revisa los campos obligatorios e inténtalo de nuevo.",
    emailDelivery: "Tus datos fueron guardados, pero falló el envío del correo. Contacta al soporte.",
  },
};
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (directData?.code === "EMAIL_DELIVERY_FAILED") {
          setSubmitError(errorText.emailDelivery);
          return;
        }
      ...
      if (!response?.ok) {
        if (response?.code === "EMAIL_DELIVERY_FAILED") {
          setSubmitError(errorText.emailDelivery);
          return;
        }
